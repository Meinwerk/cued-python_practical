...
class UMHdcSim():
    def __init__(self, domainUtil):
        ...
        if Settings.config.has_option('um', 'usenewgoalscenarios'):
            config_list.append('usenewgoalscenarios')
            self.use_new_goal_scenarios = Settings.config.getboolean('um', 'usenewgoalscenarios')
        self.answer_req_always = False
        if Settings.config.has_option('um', 'answerreqalways'):
            config_list.append('answerreqalways')
            self.answer_req_always = Settings.config.getboolean('um', 'answerreqalways')

        if Settings.config.has_section('um'):
            for opt in Settings.config.options('um'):
                if opt not in config_list and opt not in Settings.config.defaults():
                    logger.error('Invalid config: ' + opt)

        self.agenda = UserModel.UMAgenda(domainUtil)
	...
	if not len(sys_act.items): # ['slots']:
            if random.random() < rand_decision_probs['Greeting']:
                ...
                if Settings.random.rand() < rand_decision_probs['YesAfterReqmore']:
                    ...
                    for const in goal.constraints:
                        if const.val != 'dontcare':
                            slots_to_add = self.domainUtil.getSlotsToExpress(const.slot, const.val)
    ...
    def _receive_select(self, sys_act, goal):
        ...
        if not goal.contains_slot_const(slot):
            ...
            random_val = self.domainUtil.getRandomValueForSlot(slot)
    ...
    def _receive_request(self, sys_act, goal):
        ...
        if requested_slot in self.domainUtil.ontology['requestable'] and\
                requested_slot not in self.domainUtil.ontology['system_requestable']:
            self.agenda.push(DiaAct.DiaAct('negate()'))
            return
        ...
        if not self.domainUtil.is_valid_request(goal.request_type, requested_slot): # 'restaurant', requested_slot):
            self.agenda.push(DiaAct.DiaAct('inform(type="%s")' % goal.request_type))
            return
        ...
        answer_slots = self.domainUtil.getSlotsToExpress(requested_slot, requested_value)
                ...
                if len(goal.constraints) == 0:
                    ...
                    for item in sys_act.items:
                        ...
                        if item.slot not in ['name', 'type'] and\
                                item.slot in self.domainUtil.ontology['system_requestable'] and\
                    ...
                    if relax_slot is not None:
                        ...
                        if Settings.random.rand() < rand_decision_probs['ConstraintRelax'] or relax_dontcare:
                            ...
                            logger.debug("--case1--")
                            # TODO - check this - added this elif as all domains bar TT were crashing here
                            relax_value = 'dontcare'
                            ...
                            relax_value = self.domainUtil.getRandomValueForSlot(relax_slot, nodontcare=True,
    ...
    def _receive_direct_implicit_confirm(self, sys_act, goal):
        ...
        for item in sys_act.items:
            ...
            if not goal.contains_slot_const(slot) and val != 'dontcare':
                if not self.domainUtil.is_implied(slot, val):
    ...
    def _receive_implicit_confirm(self, sys_act, goal, fromconfirm=True):
        ...
        for item in sys_act.items:
            ...
            if item.slot in self.domainUtil.ontology['requestable'] and\
                    item.slot not in self.domainUtil.ontology['system_requestable'] and\
            ...
            if not contains_name_none:
                ...
                confirmable_consts = []
                for const in goal.constraints:
                    slots_to_confirm = self.domainUtil.getSlotsToExpress(const.slot, const.val)
                ...
                if len(confirmable_consts) > 0:
                    ...
                    slots_to_exp = self.domainUtil.getSlotsToExpress(rci.slot, rci.val)
    ...
    def respond(self, goal):
        ...
        :param goal: (instance) of :class:`UMGoal` 
